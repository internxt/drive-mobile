...
import { Photo, PhotoStatus } from '../sync/types';
...
class SQLiteService {
  ...
  public async getMostRecentCreationDate(): Promise<Date | null> {
    return this.pool['photos.db'].executeSql(photos.statements.getMostRecentCreationDate)
      .then((res) => {
  ...
  public async getPhotoByName(photoName: string): Promise<Photo | null> {
    return this.pool['photos.db'].executeSql(photos.statements.getPhotoByName, [photoName])
      .then((res) => {
  ...
  public async getPhotoById(photoId: PhotoId): Promise<Photo | null> {
    return this.pool['photos.db'].executeSql(photos.statements.getById, [photoId])
      .then((res) => {
        if (res[0].rows.item(0)) {
          return res[0].rows.item(0);
        } else {
  ...
  public async updatePhotoStatusById(photoId: PhotoId, newStatus: PhotoStatus): Promise<void> {
    return this.pool['photos.db'].executeSql(
      photos.statements.updatePhotoStatusById,
      [newStatus, photoId]
    ).then(() => {
      //
    });
  ...
  public async getMostRecentPullFromRemoteDate(): Promise<Date | null> {
    return this.pool['photos.db'].executeSql(
      syncDates.statements.getMostRecentPullFromRemoteDate
    ).then((res) => {
  ...
  public async updateLastPullFromRemoteDate(newDate: Date): Promise<void> {
    return this.pool['photos.db'].executeSql(
      syncDates.statements.updateByDate, [newDate.toUTCString()]
    ).then(() => {
      //
    });
  }
  ...
  public getSyncDatesCount(): Promise<number> {
    return this.pool['photos.db'].executeSql(
      syncDates.statements.selectCount
    ).then((res) => {
      if (res[0].rows.item(0)) {
  ...
  public initSyncDates() {
    return this.pool['photos.db'].executeSql(
      syncDates.statements.insert, [new Date('January 1, 1971 00:00:01')]
    );
  }
