...
async function decryptFile(encryptedFileURI: string, fileSize: number, decipher: Decipher): Promise<FileDecryptedURI> {
  ...
  return eachLimit(new Array(chunks), 1, (_, cb) => {
    RNFS.read(encryptedFileURI, twoMb, start, 'base64')
      .then((res) => {
