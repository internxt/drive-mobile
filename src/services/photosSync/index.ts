import { DeviceId } from '@internxt/sdk/dist/photos';
...
class RemotePhotosCursor {
  ...
  async next(): Promise<Photo[]> {
    const remotePhotos: Photo[] = await getRemotePhotosSince(this.lastUpdate, this.opts.limit, this.opts.offset, {
      headers: {
...
class LocalPhotosCursor {
  ...
  async next(): Promise<NewPhoto[]> {
    ...
    const [galleryPhotos, cursor] = await loadLocalPhotos(this.lastUpdate, new Date(), this.opts.limit, this.cursor);
