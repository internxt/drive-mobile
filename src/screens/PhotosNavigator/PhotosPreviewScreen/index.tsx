...
function PhotosPreviewScreen(props: PreviewProps): JSX.Element {
  ...
  */
  const photoPath = getDocumentsDir() + '/' + photo.file_id;

  const dispatch = useAppDispatch();
  const [isOptionsModalOpen, setIsOptionsModalOpen] = useState(false);
  const [uri, setUri] = useState('data:image/png;base64,' + photo.preview);
  ...
  const isPhotoAlreadyDownloaded = () => {
    return RNFS.exists(photoPath);
  };

  const loadImage = () => {
    // TODO: Get creds from store
    downloadFile('', props.route.params.data.file_id, {
      encryptionKey: '',
      pass: '',
      user: ''
    }, {
      ...
      <View style={tailwind('h-full')}>
        ...
        <SafeAreaView style={tailwind('flex-col justify-between h-full')}>
          ...
          >
            ...
            <TouchableOpacity disabled={!downloadFinished} style={tailwind('items-center')} onPress={onDownloadButtonPressed}>
