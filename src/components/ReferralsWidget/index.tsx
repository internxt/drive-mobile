      ...
      return (
        ...
        >
          <View style={[tailwind('flex-row items-center p-4'), !isTheLast && tailwind('border-b border-neutral-20')]}>
            ...
            {r.isCompleted ? (
              <Check color={tailwind('text-green-50').color as string} size={24} style={tailwind('ml-2')} />
            ) : (
              hasClickAction && (
                <CaretRight color={tailwind('text-blue-60').color as string} size={24} style={tailwind('ml-2')} />
              )
            )}
          </View>
        </TouchableHighlight>
