        ...
        <View style={styles.row}>
          ...
          >
            <View style={[styles.planStorageContainer]}>
              <Text style={[styles.planStorage]}>{plan.name}</Text>
            </View>
          </LinearGradient>
          <Text style={styles.planPrice}>
            <Text style={{ fontFamily: 'CircularStd-Bold' }}>
              {parseInt(plan.price_eur) === 0 ? 'Free' : `€${plan.price_eur}`}
            </Text>
            {plan.period ? (
              <Text style={{ color: '#7e848c' }}>/{plan.period}</Text>
            ) : (
              ''
            )}
          </Text>
